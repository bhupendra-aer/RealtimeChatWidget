...
export class MessagingService {
  ...
  urlNodeFirebaseSender: string;
  urlNodeFirebaseRecipient: string;
  ...
  public initialize(userUid, tenant, channel_type) {
    ...
    this.tenant = tenant;
    this.urlNodeFirebaseSender = '/users/' + this.senderId + '/messages/';
    this.urlNodeFirebaseRecipient= '/users/' + this.conversationWith + '/messages/';
  ...
  */
  public checkListMessages(conversationWith): any {
    ...
    const that = this;
    const firebaseMessages = firebase.database().ref(this.urlNodeFirebaseSender + this.conversationWith);
 ...
 */
  ...
  /**
   * the status of the message is arriorno
   * this status indicates that it was delivered to the client and NOT that it was read
   * if the message was NOT sent by loggedUser UPDATE was at 200
   * @param item
   * @param conversationWith
  */
  private setStatusMessage(item, conversationWith) {
    if (item.val()['status'] < MSG_STATUS_RECEIVED) {
      ...
      if (msg.sender !== this.senderId && msg.status < MSG_STATUS_RECEIVED) {
        ...
        const urlNodeMessagesUpdate  ='/users/' + this.senderId + '/messages/' + conversationWith + '/' + item.key;
        console.log('UPDATE STATE MESSAGE', urlNodeMessagesUpdate);
  ...
  sendMessage(senderFullname, msg, type, metadata, conversationWith, recipientFullname, attributes, projectid) { // : string {
    ...
    const firebaseMessagesCustomUid = firebase.database().ref(this.urlNodeFirebaseSender + conversationWith);
    const firebaseMessagesCustomUid2 = firebase.database().ref(this.urlNodeFirebaseRecipient + this.senderId);
    
    console.log('message **************', this.urlNodeFirebaseSender + conversationWith, attributes);
    console.log('message **************', this.urlNodeFirebaseRecipient + this.senderId, attributes);
    ...
    //   }
      ...
      const messageRef2 = firebaseMessagesCustomUid2.push();
      ...
      messageRef.set(message, function( error ){
        // Callback comes here
        if (error) {
          // change the status in red: send nn successful!!
          message.status = '-100';
          console.log('ERROR', error);
        } else {
          // that.checkWritingMessages();
          message.status = '150';
          console.log('OK MSG SENT WITH SUCCESS TO SERVER', message);
        }
        console.log('****** changed *****', that.messages);

      });
      messageRef2.set(message, function( error ){
        // Callback comes here
        if (error) {
          // change the status in red: send nn successful!!
          message.status = '-100';
  ...
  */
  ...
  private checkRemoveMember() {
    const that = this;
    // after adding a message to the group
    // I subscribe to the knot user/groups/ui-group/members
    // tslint:disable-next-line:max-line-length
    const urlNodeFirebaseGroupMenbers  = '/apps/' + this.tenant + '/users/' + this.senderId + '/groups/' + this.conversationWith + '/members/';
    console.log('SUBSCRIBE TO !!!!!', urlNodeFirebaseGroupMenbers);
    ...
    this.firebaseGroupMenbersRef.on('child_removed', function(childSnapshot) {
      console.log('I REMOVED!!!!!', childSnapshot.key, urlNodeFirebaseGroupMenbers);
  ...
  generateUidConversation(uid): string {
    this.firebaseMessagesKey = firebase.database().ref(this.urlNodeFirebaseSender);
    // I create the conversation node generating a custom uid
  ...
  /**
   * I purify the message
   * and I pass it to the sending method
   */
  ...
  updateMetadataMessage(uid, metadata) {
    ...
    const firebaseMessages = firebase.database().ref(this.urlNodeFirebaseSender + uid);
